...
in
{
  options = wrapInModule {
    enable = lib.mkEnableOption "all the needed to game";
  };
  ...
  config = lib.mkIf cfg.enable
  {
    ...
    /*************************** HARDWARE SUPPORT ****************************/
    # Copied from: https://gitlab.com/theblackdon/black-don-os/-/blob/main/modules/core/gaming-support.nix?ref_type=heads

    ### CONTROLLER SUPPORT ###
    hardware = {
      steam-hardware.enable = true; # udev rules for controller support
      xpadneo.enable = true; # Better Xbox controller support, especially for wireless
    };
